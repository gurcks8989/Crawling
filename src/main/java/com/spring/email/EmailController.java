 ...
 * 
 * @RequestMapping("email/*") // 공통적인 매핑 주소 public class EmailController {
 * 
 * @Inject EmailService emailService; // 서비스를 호출하기위한 의존성 주입
 * 
 ...
 * 
 * @RequestMapping("send.do") // 확인 (메일발송) 버튼을 누르면 맵핑되는 메소드 public String
 * send(@ModelAttribute EmailDTO dto, Model model) { try {
 * 
 * 
 ...
 * "이메일 발송 실패..."); // 이메일 발송이 실패되었다는 메시지를 출력 } return "/email/write"; // 실패했으므로
 * 다시 write jsp 페이지로 이동함 } }
 */